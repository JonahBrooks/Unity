...
ostream& operator<< (ostream& out, const Map& argmap) { return out << argmap; }

// This only matters if you don't set it in main.
float Map::ideal = 90;
...
void Map::regenerate_map()
{
  ...
  // Force deer to spawn in top left, exit in bottom right
	deer_index = 0; // rand() % map.size();
	exit_index = size()-1; // rand() % map.size();
...
int main()
{
  ...
  map_name_ss << "map" << Map::ideal << "_";
  map_name_base = map_name_ss.str();
	...
	while (map_found == false)
	{
		...
		for (int i = 0; i < mu + theta; i++)
		{
			...
			cout << "The map was completed " << wins << "/" << num_tries << "(" << maps[i].rating << "\%)\n";
		}
		// Sort by rating
        ...
        if(map_num >= num_maps)
        {
			...
			// Mutate mu of them over mu of them
			if (i >= mu)
			{
				maps[i] = maps[i - mu];
				maps[i].simplify_map();
